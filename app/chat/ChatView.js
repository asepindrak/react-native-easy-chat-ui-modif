...
class ChatWindow extends PureComponent {
  ...
  _changeMethod() {
    this.setState({ showVoice: !this.state.showVoice }, async () => {
      if (
        ...
        !this.androidHasAudioPermission
      ) {
  ...
  closePanel = (realClose = false, callback) => {
    Animated.parallel([
      Animated.timing(
        Platform.OS === "ios" ? this.visibleHeight : this.paddingHeight,
        {
  ...
  _onEmojiSelected = (code) => {
    ...
    if (code === "/{del}") {
      ...
      if (this.state.cursorIndex < currentTextLength) {
        ...
        if (emojiIndex === -1) {
          // 没有匹配到表情符
          const preStr = this.state.messageContent.substring(
            0,
            this.state.cursorIndex
          );
          const nextStr = this.state.messageContent.substring(
            this.state.cursorIndex
          );
        ...
        // 光标在字符串最后
        const lastChar = this.state.messageContent.charAt(
          currentTextLength - 1
        );
        if (lastChar === "]") {
      ...
      if (currentTextLength >= this.state.cursorIndex) {
        ...
        const preTemp = this.state.messageContent.substring(
          0,
          this.state.cursorIndex
        );
        ...
        >
          ...
          {this.props.showInput ? (
            <InputBar
              ...
              onSubmitEditing={(type, content) =>
                this._sendMessage(type, content)
              }
          ...
          <PanelContainer
            panelContainerBackgroundColor={
              this.props.panelContainerBackgroundColor
            }
